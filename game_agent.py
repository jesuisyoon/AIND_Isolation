...
def custom_score_2(game, player):
    ...
    if game.is_winner(player):
        return float("inf")
    regal_moves = len(game.get_legal_moves(player))
    regal_opponent_moves = len(game.get_legal_moves(game.get_opponent(player)))
    return float(regal_moves - regal_opponent_moves)
...
def custom_score_3(game, player):
    ...
    regal_moves = game.get_legal_moves(player)
    regal_opponent_moves = game.get_legal_moves(game.get_opponent(player))
    available_moves = game.get_blank_spaces()
    distance_score=0
    distance_opponent_score = 0
    for p in available_moves:
        for m in regal_moves:
        	distance_score +=abs(p[0]-m[0])+abs(p[1]-m[1])
        for k in regal_opponent_moves:
            distance_opponent_score +=abs(p[0]-k[0])+abs(p[1]-k[1])


    return float(distance_opponent_score-distance_score)
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def get_move(self, game, time_left):
        ...
        **********************************************************************
        ...
        try:
        	depth_ID = 1
        	while True:
        		best_move = self.alphabeta(game, depth_ID)
        		depth_ID += 1
    ...
    def terminal_test(self, game, depth):
        if self.time_left() < self.TIMER_THRESHOLD:
            raise SearchTimeout()
        """ Return the True value if the game is over for the active player.
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        """def alphabeta_search(self, game):
            ...
            for m in game.get_legal_moves():
                v = max_value(game, float("-inf"), float("inf"))
                if v < best_score:
                    best_score = v
                    best_move = m 
            return best_move"""
